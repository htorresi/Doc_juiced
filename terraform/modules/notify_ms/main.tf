...
/*  Author: hgb #67 9/13/21
    Description: Trigger the alert message to MS-Team using AWS SNS and Lambda function Example: https://medium.com/javarevisited/trigger-the-alert-message-to-ms-team-using-aws-sns-and-lambda-function-1c7769012eff
*/
...
locals {
    sns_topic_arn = var.create_sns_topic == false ? var.sns_topic_arn: element(
        concat(
...
resource "aws_sns_topic_subscription" "sns_notify_ms" {
    count = var.create ? 1 : 0
...
module "lambda" {
    ...
    handler                        = "notify_ms.lambda_handler"
    source_path                    = "${path.module}/functions/notify_ms.py"
    runtime                        = "python3.8"
    timeout                        = 30
    kms_key_arn                    = var.kms_key_arn
    reserved_concurrent_executions = var.reserved_concurrent_executions

    # If publish is disabled, there will be "Error adding new Lambda Permission for notify_ms: InvalidParameterValueException: We currently do not support adding policies for $LATEST."
    publish = true
